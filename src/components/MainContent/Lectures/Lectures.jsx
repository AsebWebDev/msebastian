  ...
  return (
    <Container text id="lectures">
      <Header inverted={inverted} as="h1">
        {dict[lang].LECTURES.LECTURES_TITLE}
      </Header>

      {/* Vorträge auf wissenschaftlichen Tagungen */}
      <Header color="blue" inverted={inverted} as="h2">
        {dict[lang].LECTURES.LECTURES_1_HEADER}
      </Header>
      <List inverted={inverted} divided relaxed>
        {dict[lang].LECTURES.LECTURES_1_ITEMS.map((lecture, i) => (
          <ListItem
            key={i} // TODO: add proper key to avoid using index
            header={lecture.dateandvenue}
            metaText={lecture.title}
          />
        ))}
      </List>
      ...
      <Header color="blue" inverted={inverted} as="h2">
        {dict[lang].LECTURES.LECTURES_2_HEADER}
      </Header>
      <List inverted={inverted} divided relaxed>
        {dict[lang].LECTURES.LECTURES_2_ITEMS.map((lecture, i) => (
          <ListItem
            key={i} // TODO: add proper key to avoid using index
            header={lecture.dateandvenue}
            metaText={lecture.title}
